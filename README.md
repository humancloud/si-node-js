# StackInsights NodeJS Agent

**StackInsights-NodeJS**: The NodeJS Agent for StackInsights, which provides the native tracing abilities for NodeJS backend project.

**StackInsights**: an APM(application performance monitor) system, especially designed for
microservices, cloud native and container-based (Docker, Kubernetes, Mesos) architectures.


## Install StackInsights NodeJS package from npmjs

```bash
$ npm install --save stackinsights-backend-js
```

## Set up NodeJS Agent

StackInsights NodeJS SDK requires StackInsights backend (OAP) 8.0+ and NodeJS >= 10.
...
This will use default configurations to start the StackInsights agent above, if you want to specify your own configurations, here are two methods.
...
| `SW_AGENT_AUTHENTICATION` | The authentication token to verify that the agent is trusted by the backend OAP, as for how to configure the backend, refer to [the yaml](https://github.com/apache/stackinsights/blob/4f0f39ffccdc9b41049903cc540b8904f7c9728e/oap-server/server-bootstrap/src/main/resources/application.yml#L155-L158). | not set |
...
```javascript
const {default: agent, AWSLambdaGatewayAPIHTTP} = require('stackinsights-backend-js');
